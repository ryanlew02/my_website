...
// ─── ASCII portrait ─────────────────────────────────────────────────────────
// Replaces the hero photo with a character-grid rendering of it that turns to
// follow the pointer while the pointer is over the plate, and eases back to
// facing front when it leaves.
//
// The data (assets/portrait-ascii.js, generated by tools/portrait-ascii) is a
// 96x77 grid of tone bytes plus a per-cell depth map estimated from the photo
// by Depth Anything V2. That makes the portrait a height field, not a picture:
// each cell is a point in 3D that gets rotated, projected and z-buffered into
// the character grid every frame, exactly the way the spinning-donut demos
// work. What you get is real parallax — the nose leads the cheeks, the near
// side of the face occludes the far side — rather than a skewed flat image.
//
// A photograph has no back, though, and on the height field alone the
// silhouette tore open past about 25°. So the sides are invented: buildWrap()
// continues every row of the head around past the silhouette as an elliptical
// slice and models an ear onto it. That is a guess wearing the tone carried
// out from the last cells the camera did see — good for the ~34° the rotation
// is capped at, and nowhere near a profile.
(function () {
    var data = window.PORTRAIT_ASCII;
    var photo = document.querySelector('.id-photo');
    if (!data || !photo) return;

    // The grid's type is sized in container units off the plate. Without them
    // the glyphs would fall back to the inherited size and blow out of the
    // frame, so leave the photo in place instead.
    if (!window.CSS || !CSS.supports || !CSS.supports('width', '1cqw')) return;

    var COLS = data.cols, ROWS = data.rows, RAMP = data.ramp;
    var LEVELS = RAMP.length - 1;

    // Cell size in world units, where 1 = the plate's width. The grid is 3:4,
    // so cells are taller than they are wide and the two steps differ.
    var DX = 1 / COLS, DY = (4 / 3) / ROWS;

    var CAM = 2.8;          // camera distance, in plate widths
    var FIT = 0.93;         // shrink slightly so a turned head never clips
    var SHADE_MIX = 0.28;   // how far the lambert term is allowed to move tone
    var MAX_YAW = 0.60;     // ~34°. The invented sides are what buy this: on
                            // the photographed height field alone the silhouette
                            // tore open past about 25°.
    var MAX_PITCH = 0.14;   // ~8°, and no more — there is no data above the brow
    var EASE = 0.085;       // per-frame approach to the pointer's target

    // Ordered dithering. The face is a smooth gradient and the ramp has ten
    // steps, so without this the tone quantises into visible horizontal bands
    ...
    function zAt(c, r) {
        ...
        var idx = r * COLS + c;
        return lum[idx] ? (dep[idx] / 255) * data.zDepth : NaN;
    }

    function slope(c, r, dc, dr, step) {
        var a = zAt(c + dc, r + dr), b = zAt(c - dc, r - dr), z = zAt(c, r);
        if (a === a && b === b) return (a - b) / (2 * step);
        if (a === a) return (a - z) / step;
        if (b === b) return (z - b) / step;
        return 0;
    }

    // ── The sides the camera never saw ──────────────────────────────────────
    // A photograph has no back, so past a shallow turn the height field runs
    // out of surface and the head shows its cut edge. This invents the missing
    // sides: every row of the head is read as a horizontal slice through the
    // skull, and the ellipse the photo gives the front of is continued round
    // past the silhouette. The ear is a modelled bump on that new surface, put
    // where an ear goes — brow to nose-base, at the widest part of the skull.
    //
    ...
    function buildWrap() {
        ...
        var K = Math.round(w.deg / w.step) + 1;
        var widths = [], r, c, cnt, cL, cR;
        for (r = 0; r < ROWS; r++) {
            cnt = 0;
            for (c = 0; c < COLS; c++) if (lum[r * COLS + c]) cnt++;
            widths.push(cnt);
        }
        var live = widths.filter(function (v) { return v >= 6; }).sort(function (a, b) { return a - b; });
        if (!live.length) return null;
        // Rows no wider than the head. Shoulders are elliptical too, but four
        // times across, and continuing them invents a torso nobody asked for.
        var maxWidth = 1.3 * live[live.length >> 1];

        var slices = {}, order = [];
        for (r = 0; r < ROWS; r++) {
            if (widths[r] < 6 || widths[r] > maxWidth) continue;
            cL = -1; cR = -1;
            for (c = 0; c < COLS; c++) if (lum[r * COLS + c]) { if (cL < 0) cL = c; cR = c; }
            var xL = (cL + 0.5 - COLS / 2) * DX, xR = (cR + 0.5 - COLS / 2) * DX;
            ...
            var cFront = Math.max(zFront - zEdge, 0.05 * a);
            var cBack = Math.max(w.back * a, cFront);
            var y = (r + 0.5 - ROWS / 2) * DY;

            for (var s = 0; s < 2; s++) {
                var side = s ? 1 : -1;
                // Tone to carry outward: the last few cells at that edge.
                var t0 = side < 0
                    ? (lum[r * COLS + cL] + lum[r * COLS + Math.min(cL + 1, COLS - 1)] + lum[r * COLS + Math.min(cL + 2, COLS - 1)]) / 765
                    : (lum[r * COLS + cR] + lum[r * COLS + Math.max(cR - 1, 0)] + lum[r * COLS + Math.max(cR - 2, 0)]) / 765;
                var pos = new Float32Array(K * 3);
                for (k = 0; k < K; k++) {
                    var deg = 90 + k * w.step, t = deg * Math.PI / 180;
                    var st = Math.sin(t), ct = Math.cos(t);
                    var q = Math.min(1, (deg - 90) / w.blend);
                    var cz = cFront + (cBack - cFront) * (q * q * (3 - 2 * q));
                    // Ear: a rounded, slightly teardrop bump along the normal.
                    var bump = 0;
                    if (r >= w.earRows[0] && r <= w.earRows[1] &&
                        deg >= w.earAng[0] && deg <= w.earAng[1]) {
                        var u = (r - w.earRows[0]) / (w.earRows[1] - w.earRows[0]) * 2 - 1;
                        var v = (deg - w.earAng[0]) / (w.earAng[1] - w.earAng[0]) * 2 - 1;
                        bump = w.earOut * (1 - 0.22 * u) *
                            Math.pow(Math.cos(u * Math.PI / 2), 1.3) *
                            Math.pow(Math.cos(v * Math.PI / 2), 1.1);
                    }
                    var ox = cz * st, oz = a * ct, ol = Math.sqrt(ox * ox + oz * oz) || 1;
                    pos[k * 3] = xc + side * (a * st + bump * ox / ol);
                    pos[k * 3 + 1] = y;
                    pos[k * 3 + 2] = zEdge + cz * ct + bump * oz / ol;
                }
                slices[r + ':' + s] = { pos: pos, tone: t0, a: a, side: side };
                order.push(r + ':' + s);
            }
        }

        // Normals from finite differences across the (row, angle) surface, so
        // the ear reads as geometry and catches the light like one.
        var out = { x: [], y: [], z: [], nx: [], ny: [], nz: [], t: [], s: [] };
        for (var oi = 0; oi < order.length; oi++) {
            var key = order[oi], parts = key.split(':');
            var row = +parts[0], sIdx = +parts[1], sl = slices[key];
            var up = slices[(row - 1) + ':' + sIdx], dn = slices[(row + 1) + ':' + sIdx];
            if (!up && !dn) continue;
            for (k = 0; k < K; k++) {
                var kA = Math.min(k + 1, K - 1) * 3, kB = Math.max(k - 1, 0) * 3;
                var ax = sl.pos[kA] - sl.pos[kB];
                var ay = sl.pos[kA + 1] - sl.pos[kB + 1];
                var az = sl.pos[kA + 2] - sl.pos[kB + 2];
                var other = dn || up, sign = dn ? 1 : -1;
                var bx = (other.pos[k * 3] - sl.pos[k * 3]) * sign;
                var by = (other.pos[k * 3 + 1] - sl.pos[k * 3 + 1]) * sign;
                var bz = (other.pos[k * 3 + 2] - sl.pos[k * 3 + 2]) * sign;
                if (up && dn) {
                    bx = dn.pos[k * 3] - up.pos[k * 3];
                    by = dn.pos[k * 3 + 1] - up.pos[k * 3 + 1];
                    bz = dn.pos[k * 3 + 2] - up.pos[k * 3 + 2];
                }
                var vx = by * az - bz * ay;
                var vy = bz * ax - bx * az;
                var vz = bx * ay - by * ax;
                ...
                // Orient outward using the analytic ellipse normal.
                var degK = 90 + k * w.step, tK = degK * Math.PI / 180;
                if (vx * (sl.side * Math.sin(tK)) + vz * Math.cos(tK) < 0) {
                    vx = -vx; vy = -vy; vz = -vz;
                }
                var frac = k / (K - 1);
                out.x.push(sl.pos[k * 3]);
                out.y.push(sl.pos[k * 3 + 1]);
                out.z.push(sl.pos[k * 3 + 2] - data.zMid);
                out.nx.push(vx); out.ny.push(vy); out.nz.push(vz);
                out.t.push(sl.tone * (1 - (1 - w.fade) * frac));
                out.s.push(Math.max(-6, Math.min(6, -Math.tan(tK))));
            }
        }
        return out;
    }
    ...
    // One point per drawn cell, plus the invented sides. Photographed normals
    // come from the depth gradient, one-sided at the silhouette so the drop to
    // the background doesn't register as a cliff-face.
    var i, j, k;
    var nPhoto = 0;
    for (i = 0; i < lum.length; i++) if (lum[i]) nPhoto++;

    var wrap = buildWrap();
    var nPts = nPhoto + (wrap ? wrap.t.length : 0);

    var px = new Float32Array(nPts), py = new Float32Array(nPts), pz = new Float32Array(nPts);
    var nx = new Float32Array(nPts), ny = new Float32Array(nPts), nz = new Float32Array(nPts);
    var tone = new Float32Array(nPts);
    var dzdx = new Float32Array(nPts);   // kept for the per-point splat width
    ...
    for (j = 0; j < ROWS; j++) {
        for (i = 0; i < COLS; i++) {
            var idx = j * COLS + i;
            if (!lum[idx]) continue;
            var sx = slope(i, j, 1, 0, DX);
            var sy = slope(i, j, 0, 1, DY);
            var len = Math.sqrt(sx * sx + sy * sy + 1);
            px[k] = (i + 0.5 - COLS / 2) * DX;
            py[k] = (j + 0.5 - ROWS / 2) * DY;
            pz[k] = (dep[idx] / 255) * data.zDepth - data.zMid;
            nx[k] = -sx / len;
            ny[k] = -sy / len;
            nz[k] = 1 / len;
            tone[k] = lum[idx] / 255;
    ...
    if (wrap) {
        for (i = 0; i < wrap.t.length; i++, k++) {
            px[k] = wrap.x[i]; py[k] = wrap.y[i]; pz[k] = wrap.z[i];
            nx[k] = wrap.nx[i]; ny[k] = wrap.ny[i]; nz[k] = wrap.nz[i];
            tone[k] = wrap.t[i];
            dzdx[k] = wrap.s[i];
        }
    }
    ...
    // Seven thousand punctuation marks are not a reading experience. The plate
    // carries the name instead, since hiding the <img> takes its alt with it.
    pre.setAttribute('aria-hidden', 'true');
    photo.appendChild(pre);
    photo.setAttribute('role', 'img');
    photo.setAttribute('aria-label', 'Ryan Lewandowski, drawn as ASCII art');
    photo.classList.add('is-ascii');

    var glyph = new Uint8Array(COLS * ROWS);   // 0 = empty, else index into RAMP
    var zbuf = new Float32Array(COLS * ROWS);
    var rowChars = new Array(COLS);
    ...
    function draw(yaw, pitch) {
        ...
        // Shading is dot(R·normal, light). Rotating the light by R⁻¹ once
        // instead gives the same number for nine fewer multiplies per point.
        var L = data.light;
        var ly = L[1] * cb + L[2] * sb;
        var lz = -L[1] * sb + L[2] * cb;
        var lx = L[0] * ca - lz * sa;
        lz = L[0] * sa + lz * ca;

        var span = data.shadeHi - data.shadeLo;
        var half = COLS / 2, halfR = ROWS / 2;
        var m, x, y, z, zr, persp, col, row, cell, lam, t, v, g;
        var facing, wide, spread, c0, c1, c;

        glyph.fill(0);
        zbuf.fill(-1e9);

        for (m = 0; m < nPts; m++) {
            // Backface cull first: at the silhouette the normals already point
            // sideways, and once rotated past edge-on those cells smear across
            // the far side of the face as loose speckle.
            facing = ny[m] * sb + (nz[m] * ca - nx[m] * sa) * cb;
            ...
            persp = CAM / (CAM - z);
            col = (x * persp * FIT) / DX + half - 0.5;
            row = (y * persp * FIT) / DY + halfR - 0.5;
            col = col < 0 ? (col - 0.5) | 0 : (col + 0.5) | 0;
            row = row < 0 ? (row - 0.5) | 0 : (row + 0.5) | 0;
            if (row < 0 || row >= ROWS) continue;

            lam = (nx[m] * lx + ny[m] * ly + nz[m] * lz - data.shadeLo) / span;
            lam = lam < 0 ? 0 : lam > 1 ? 1 : lam;
            t = tone[m] * (1 - SHADE_MIX + SHADE_MIX * 2 * lam);

            // How far apart this cell and its neighbour land once turned. Where
            // the surface tips away from the camera the grid pulls apart and a
            // one-cell mark leaves a gap, so the point is smeared over the span
            // it actually covers. Falls back to a single cell when the surface
            // is compressing instead (|spread| < 1), which is the common case.
            spread = Math.abs(ca + dzdx[m] * sa) * persp * FIT;
            wide = spread > 1.4 ? (spread + 0.5) | 0 : 1;
            if (wide > 4) wide = 4;
            c0 = col - (wide >> 1);
            c1 = c0 + wide;
            ...
            for (c = c0; c < c1; c++) {
                cell = row * COLS + c;
                if (persp <= zbuf[cell]) continue;
                zbuf[cell] = persp;
                v = Math.round(t * LEVELS + BAYER[(row & 3) * 4 + (c & 3)]);
                glyph[cell] = v < 1 ? 1 : v > LEVELS ? LEVELS : v;
            }
        }

        // Backstop for the gaps the splat above doesn't close — mostly single
        // cells at the silhouette. Filled from whichever neighbour is nearer,
        // so a fill never bleeds the far side of the face over the near one.
        for (row = 0; row < ROWS; row++) {
            var base = row * COLS;
            for (col = 1; col < COLS - 1; col++) {
                cell = base + col;
                if (glyph[cell] || !glyph[cell - 1] || !glyph[cell + 1]) continue;
                g = zbuf[cell - 1] > zbuf[cell + 1] ? cell - 1 : cell + 1;
                glyph[cell] = glyph[g];
                zbuf[cell] = zbuf[g];
            }
        }
        ...
        for (row = 0; row < ROWS; row++) {
            for (col = 0; col < COLS; col++) rowChars[col] = RAMP[glyph[row * COLS + col]];
    ...
    var drawnYaw = 0, drawnPitch = 0;   // set by draw(), never NaN
    var visible = true, hovering = false, frame = 0;

    // Only the plate itself steers the head. Tracking the pointer across the
    // whole page meant the portrait was never still and never obviously
    // interactive — it just twitched at whatever you were actually doing.
    // Scoped here it reads as direct manipulation, and the mapping can be the
    // plate's own box: full turn by the time you reach the frame.
    function onMove(e) {
        if (!canHover) return;
        ...
        targetYaw = MAX_YAW * dx;
        targetPitch = MAX_PITCH * dy;
        start();
    }
    ...
    function tick(now) {
        ...
        // A touch screen has no pointer to follow, so there the head drifts on
        // its own rather than sitting frozen with no way to move it.
        if (!canHover) {
            targetYaw = MAX_YAW * 0.42 * Math.sin(now / 4200);
            targetPitch = MAX_PITCH * 0.35 * Math.sin(now / 6100);
        }

        yaw += (targetYaw - yaw) * EASE;
        pitch += (targetPitch - pitch) * EASE;

        // The grid is only rebuilt when the angle has moved a visible amount,
        // so the frames spent easing into rest are nearly free.
        if (Math.abs(yaw - drawnYaw) > 0.0012 || Math.abs(pitch - drawnPitch) > 0.0012) {
            draw(yaw, pitch);
        }

        // Back at rest with nothing to follow: snap to dead centre and stop.
        // An untouched page shouldn't be running an animation loop at all.
        if (canHover && !hovering && Math.abs(yaw) < 0.002 && Math.abs(pitch) < 0.002) {
            yaw = pitch = 0;
            if (drawnYaw !== 0 || drawnPitch !== 0) draw(0, 0);
            return;
        }
    ...
    if (window.IntersectionObserver) {
        new IntersectionObserver(function (entries) {
            ...
            // Scrolling away with the head turned leaves it mid-turn; treat
            // that as a leave so it eases back to centre on the way out.
            if (visible) start();
            else if (frame) { cancelAnimationFrame(frame); frame = 0; onLeave(); }
    ...
    // A touch screen never hovers, so it needs the loop running from the start.
    if (!canHover) start();
})();
